...
func setupMutableTree(t *testing.T, skipFastStorageUpgrade bool) *MutableTree {
	memDB := db.NewMemDB()
	tree, err := NewMutableTree(memDB, 0, skipFastStorageUpgrade, nil, false)
...
func TestMutableTree_LazyLoadVersion_Empty(t *testing.T) {
	memDB := db.NewMemDB()
	tree, err := NewMutableTree(memDB, 0, false, nil, false)
	require.NoError(t, err)
...
func TestMutableTree_InitialVersion(t *testing.T) {
	memDB := db.NewMemDB()
	tree, err := NewMutableTreeWithOpts(memDB, 0, &Options{InitialVersion: 9}, false, nil, false)
	require.NoError(t, err)
	...
	tree, err = NewMutableTreeWithOpts(memDB, 0, &Options{InitialVersion: 10}, false, nil, false)
	require.NoError(t, err)
	_, err = tree.Load()
	require.Error(t, err)
	...
	tree, err = NewMutableTreeWithOpts(memDB, 0, &Options{InitialVersion: 3}, false, nil, false)
	require.NoError(t, err)
	version, err = tree.Load()
	require.NoError(t, err)
	assert.EqualValues(t, 10, version)
...
func prepareTree(t *testing.T) *MutableTree {
	mdb := db.NewMemDB()
	tree, err := NewMutableTree(mdb, 1000, false, nil, false)
	require.NoError(t, err)
	for i := 0; i < 100; i++ {
	...
	require.NoError(t, err)
	newTree, err := NewMutableTree(mdb, 1000, false, nil, false)
	require.NoError(t, err)
...
func TestMutableTree_LazyLoadVersionWithEmptyTree(t *testing.T) {
	...
	newTree1, err := NewMutableTree(mdb, 1000, false, nil, false)
	require.NoError(t, err)
	v2, err := newTree1.LazyLoadVersion(1)
	require.NoError(t, err)
	require.True(t, v1 == v2)

	newTree2, err := NewMutableTree(mdb, 1000, false, nil, false)
	require.NoError(t, err)
...
func TestMutableTree_FastNodeIntegration(t *testing.T) {
	...
	// Load
	t2, err := NewMutableTree(mdb, 0, false, nil, false)
	require.NoError(t, err)
...
func TestUpgradeStorageToFast_LatestVersion_Success(t *testing.T) {
	// Setup
	db := db.NewMemDB()
	tree, err := NewMutableTree(db, 1000, false, nil, false)
	require.NoError(t, err)
...
func TestUpgradeStorageToFast_DbErrorConstructor_Failure(t *testing.T) {
	...
	tree, err := NewMutableTree(dbMock, 0, false, nil, false)
	require.Nil(t, err)
...
func TestUpgradeStorageToFast_Integration_Upgraded_FastIterator_Success(t *testing.T) {
	...
	sut, _ := NewMutableTree(tree.ndb.db, 1000, false, nil, false)
...
func setupTreeAndMirror(t *testing.T, numEntries int, skipFastStorageUpgrade bool) (*MutableTree, [][]string) {
	...
	tree, _ := NewMutableTree(db, 0, skipFastStorageUpgrade, nil, false)
...
func TestNoFastStorageUpgrade_Integration_SaveVersion_Load_Get_Success(t *testing.T) {
	...
	sut, _ := NewMutableTree(tree.ndb.db, 1000, true, nil, false)
